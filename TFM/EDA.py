...
def eda_raw(df):
    ...
    # Missing en crudo
    print("\nRegistros con NaN en 'Close':", df['Close'].isna().sum())
    return counts

# ---------------- EDA PIVOT ----------------
def eda_pivot(df_pivot, max_nan_ratio=0.4, export_path="stock_weekly_clean.csv"):
    """EDA sobre la matriz pivotada + limpieza + exportación"""

    # Resample semanal
    df_pivot = df_pivot.resample('W').last()

    print("\n=== EDA Pivot (serie temporal por empresa) ===")
    print(f"Dimensiones: {df_pivot.shape}")
    print(f"Rango de fechas: {df_pivot.index.min().date()} → {df_pivot.index.max().date()}")
    ...
    print(df_pivot.describe().T.head(10))
    print("\n--- Estadísticas descriptivas (todas las empresas empresas) ---")
    ...
    print(f"\nEjemplos de descomposición estacional: {examples}")
    for ex in examples:
        series = df_pivot[ex].dropna()
        decomposition = seasonal_decompose(series, period=52)
        decomposition.plot()
        plt.suptitle(f"Descomposición estacional de {ex}", fontsize=14)
        plt.show()
...
if __name__ == "__main__":
    ...
    # EDA pivot + limpieza + exportación
    df_clean = eda_pivot(df_pivot, max_nan_ratio=0.4, export_path="stock_weekly_clean.csv")
